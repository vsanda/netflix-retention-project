...
def load_csv_to_postgres():
    ...
    videos.to_sql("videos", engine, index=False, if_exists="replace")

    # Load viewing_history.csv
    viewing_history = pd.read_csv("data/raw/viewing_history.csv")
    viewing_history.to_sql("viewing_history", engine, index=False, if_exists="replace")

    # Load subscriptions.csv
    subscriptions = pd.read_csv("data/raw/subscriptions.csv")
    subscriptions.to_sql("subscriptions", engine, index=False, if_exists="replace")

    # Load support_tickets.csv
    support_tickets = pd.read_csv("data/raw/support_tickets.csv")
    support_tickets.to_sql("support_tickets", engine, index=False, if_exists="replace")

    # Load churn_labels.csv
    churn_labels = pd.read_csv("data/raw/churn_labels.csv")
    churn_labels.to_sql("churn_labels", engine, index=False, if_exists="replace")
